...
def profile(request):
    ...
    if g == "Patient":
        ...
        reception_detials = Receptionist.objects.all().filter(email=request.user)
        d = { 'reception_detials':reception_detials}
...
def viewappointments(request):
    ...
    if g == "Patient":
        ...
        previous_appointments = Appointment.objects.filter(patientemail=request.user,appointmentdate__lt = timezone.now()).order_by('-appointmentdate') | Appointment.objects.filter(status=False).order_by('-appointmentdate')
        d = {'upcomming_appointments': upcomming_appointments,'previous_appointments': previous_appointments}
